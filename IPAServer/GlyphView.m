...
-(void)drawRect:(NSRect)r
{
  if (!_cell)
  {
    _cell = [[NSImageCell alloc] initImageCell:nil];
    [_cell setImageFrameStyle:NSImageFrameGrayBezel];
  }
  [_cell drawWithFrame:r inView:self];
  if ([_stringValue length])
  {
    ...
    OSStatus err = TRRenderText([[NSGraphicsContext currentContext] graphicsPort], cgr,
                                (CFStringRef)_stringValue, (CFStringRef)_font,
                                _fontSize, _fallbackBehavior, _baseline);
